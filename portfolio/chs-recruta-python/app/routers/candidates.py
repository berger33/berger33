...
def list_candidates(q: str = Query(default=""), db: Session = Depends(get_db)):
    return search_candidates(db, q)


@router.post("", response_model=CandidateRead, status_code=201)
def create_candidate(payload: CandidateCreate, allow_duplicate: bool = False, db: Session = Depends(get_db)):
    ...
    db.add(candidate)
    db.flush()
    audit(db, action="create", entity="candidate", entity_id=str(candidate.id), details=candidate.name)
    db.commit()
...
def update_candidate(candidate_id: int, payload: CandidateCreate, db: Session = Depends(get_db)):
    ...
    audit(db, action="update", entity="candidate", entity_id=str(candidate.id), details=candidate.name)
    db.commit()
    db.refresh(candidate)
    return candidate
...
def delete_candidate(candidate_id: int, db: Session = Depends(get_db)):
    ...
    if not candidate:
        raise HTTPException(status_code=404, detail="Candidato não encontrado")
    audit(db, action="delete", entity="candidate", entity_id=str(candidate.id), details=candidate.name)
    db.delete(candidate)
    db.commit()


@router.get("/{candidate_id}/matches")
def candidate_matches(candidate_id: int, db: Session = Depends(get_db)):
    candidate = db.get(Candidate, candidate_id)
    if not candidate:
        raise HTTPException(status_code=404, detail="Candidato não encontrado")
