...
from ..models import AuditLog, Candidate, FinancialReference
...
def list_financial(db: Session = Depends(get_db)):
...
def create_financial(payload: FinancialCreate, db: Session = Depends(get_db)):
    item = FinancialReference(**payload.model_dump())
    db.add(item)
    db.flush()
    audit(db, action="create", entity="financial", entity_id=str(item.id), details=item.service)
    db.commit()
    db.refresh(item)
    return item
