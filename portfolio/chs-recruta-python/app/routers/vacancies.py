...
def list_vacancies(_: User = Depends(current_user), db: Session = Depends(get_db)):
...
def create_vacancy(payload: VacancyCreate, user: User = Depends(current_user), db: Session = Depends(get_db)):
...
def update_vacancy(vacancy_id: int, payload: VacancyCreate, user: User = Depends(current_user), db: Session = Depends(get_db)):
    ...
    audit(db, action="update", entity="vacancy", entity_id=str(vacancy.id), actor=user.username, details=vacancy.code)
    db.commit()
    db.refresh(vacancy)
    return vacancy
